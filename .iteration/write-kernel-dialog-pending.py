...
# remove previous automation pendings if present
...
remove = [i for i in old]  # clear all previous pending for clean slate
op = {"schema_version": 1, "upsert": pendings, "remove": remove}
(root/".iteration"/"kernel-dialog-pending-op.json").write_text(json.dumps(op, ensure_ascii=False, indent=2), encoding="utf-8")
apply_operation(root/"docs"/"REQUIREMENTS-SPEC.md", root/"docs"/"PENDING-REQUIREMENTS.md", op, evidence=None)
...
(root/".iteration"/"intake-decision-kernel-dialogs.json").write_text(json.dumps(decision, ensure_ascii=False, indent=2), encoding="utf-8")
r = subprocess.run([
  sys.executable, str(skill/"scripts"/"requirements_intake.py"), "build",
  "--request-file", str(root/".iteration"/"request.txt"),
  "--decision", str(root/".iteration"/"intake-decision-kernel-dialogs.json"),
  "--intake-file", str(root/".iteration"/"intakes"/"INTAKE-20260731-KERNEL-DIALOGS.json"),
