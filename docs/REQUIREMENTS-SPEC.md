...
| --- | --- | --- | --- | --- | --- |
| v0.3.64 | 2026-08-04 | `<ITERATION-160-KERNEL-GIT-DIFF-SUMMARY>` | Desktop `git_diff_summary` now travels through authenticated kernel `/v1/domain/git/diff-summary`; repository detection precedes `git diff`, non-Git roots return a typed negative result, and Tauri remains an adapter | Advances `REQ-RIDGE-KERNEL-DOMAIN-01`, SCM process lifecycle, and startup performance; Git mutations/graph, physical Remote/WebView2, dual-window/Host, and complete kernel authority evidence remain open | Kernel diff-summary decoder/domain tests passed; ridge 256, kernel 39, core 315; Vitest 148/1541/1; `pnpm check` 0/0 |
