...
- Current iteration evidence:`Kernel/Teammate SSOT、typed envelope、generation/lease fencing、SQLite Hub、MCP tools、adapter probe/priority/outcome、desktop/Remote projections 与 lifecycle E2E 已有确定性证据；Kernel/桌面/headless 已接入容量 256、try_send、generation/lease fencing 的 host-owned Runtime API/A2A registry。共享 `GET /api/v1/agent-events/ws` bridge 现已支持 token + 当前 roster 身份授权、跨进程注册、真实 Hub envelope 投递、ACK 与断连注销，等价 multi-Agent TCP E2E 已通过；第三方 CLI 私有 Runtime/A2A 协议仍未冒充或宣称兼容。Kernel PTY destroy 会注销被移除 identity 的路由；桌面生产路径尚未将五条件从 PTY/审批/前台进程/用户输入同一运行时读模型原子发布到 Hub，故 PTY 继续 fail-closed；Sonar coverage 已达 80.2%，Quality Gate 因 new_violations=152 尚未闭环。最新测试与差距见 docs/iterations/2026-08-10-iteration-handoff.md。`
