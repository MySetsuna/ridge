...
- Current evidence:the input/RPC path already has `PaneRpcScheduler` and `paneInputGate` bounds, while `TerminalManager` time-slices PTY parsing. `terminalFeedPolicy` now caps deferred bytes at `MAX_FEED_DEFERRED_BYTES`; both append and bounded-drain remainder requeue use capped admission, so no remainder path writes an unbounded queue. Focused manager/policy regression tests pass `25/25`, and `pnpm check` reports `0 errors / 0 warnings`. Mobile/PWA and public Remote soak evidence remains external.
...
- 状态:`ACTIVE`
- 版本:`v1`
- Current evidence:`CloudRemoteConnection` 先注册 pane live listener，再执行 bounded visual seed；live bytes 立即发出并以 `REMOTE_SEED_REPLAY_MAX_BYTES` 保留有限 FIFO，seed 完成后按序 replay，overflow 则放弃 stale seed；旧 scrollback 仅走独立向上分页。`cloudRemote.test.ts` 通过 `52/52`。真机/PWA、公网 soak 仍属外部证据。
...
- Current iteration evidence:`最新成功分析为 `c271e74b-ac3f-4277-bbef-74418f48b822`：其产品源范围扫描 project coverage 80.4%、new_coverage 80.1% OK、new_duplicated_lines_density 0.84181% OK，但 Quality Gate ERROR，new_violations 1/0；旧的 `rust:S3776` 已由 `254a98e3` 重构，当前源码已含 HTML title，尚待新分析确认关闭。随后一次临时认证扫描已验证 admin authentication `valid=true`，但 token 已撤销，scanner 卡在 `Resolving provided TSConfig files`，未形成 CE task；根因是 `packages/rg-split/examples/tsconfig.json` 缺少自有 `.svelte-kit/tsconfig.json`，已由 `4270dc55` 从 Sonar 类型感知配置移除，剩余三份配置均可解析。完整源集纳入 `scripts` 的历史扫描 coverage 66.3%，不能以产品源范围结果冒充全项目达标；需新 token 重扫确认 project metric、Quality Gate OK 与 new issues=0，需求仍 ACTIVE。详见 `docs/iterations/2026-08-11-wave86-sonar-authenticated-stall.md`。`
...
- Current iteration evidence:`Kernel/Teammate SSOT、typed envelope、generation/lease fencing、SQLite Hub、MCP tools、adapter probe/priority/outcome、desktop/Remote projections 与 lifecycle E2E 已有确定性证据；Kernel/桌面/headless 已接入容量 256、try_send、generation/lease fencing 的 host-owned Runtime API/A2A registry。共享 `GET /api/v1/agent-events/ws` bridge 现已支持 token + 当前 roster 身份授权、跨进程注册、真实 Hub envelope 投递、ACK 与断连注销，等价 multi-Agent TCP E2E 已通过；Kernel `50/50`、MCP `90/90`、kernel lifecycle E2E `3/3`。未验证 CLI profile 的 resume 已 fail-closed，Remote deferred feed/remainder 具字节上限与 FIFO fencing；第三方 CLI 私有 Runtime/A2A 协议仍未冒充或宣称兼容。Kernel PTY destroy 会注销被移除 identity 的路由；桌面生产路径尚未将五条件从 PTY/审批/前台进程/用户输入同一运行时读模型原子发布到 Hub，故 PTY 继续 fail-closed。Sonar 最新可读取成功分析 `c271e74b-ac3f-4277-bbef-74418f48b822` 为 product coverage `80.4%`、new coverage `80.1%`、new duplication `0.84181%`，但 Gate `ERROR`、new violations `1/0`；Wave86 已用当前凭据认证成功并尝试重扫，但扫描卡在 TSConfig 解析且未形成新 CE/Gate 结果，故仍不得宣称质量门通过。`
