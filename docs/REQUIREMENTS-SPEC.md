...
| v0.3.77 | 2026-08-05 | `<ITERATION-171-CLOUD-DUAL-LANE>` | Cloud Remote now uses an optional `ridge-pane` ordered DataChannel on the same authenticated PeerConnection for PTY bulk output; `ridge` remains the control/input lane, each lane has independent E2EE counters and chunk IDs, and legacy peers fall back to `ridge` | Removes the remaining ordered-channel head-of-line path for `REQ-REMOTE-LINK-FLUIDITY-01` and `REQ-MOBILE-REMOTE-BACKPRESSURE-01`; code `321a5d3` is pushed but not artifact-published today because the daily release cap is exhausted; physical phone/PWA soak and online activation remain open | Host/controller routing, controller outbound lane, and legacy-fallback tests; full Vitest 154/1583/1; `pnpm check` 0/0; both mobile and desktop Remote builds passed |
