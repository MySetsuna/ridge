...
| v0.3.72 | 2026-08-05 | `<ITERATION-167-RELEASE-GATE-REMEDIATION>` | Release test gate now builds the target `rdg` sidecar before Linux Tauri compile validation, preventing the known missing `externalBin` resource failure | Closes the deterministic CI blocker only; versioned release still requires the full matrix and asset audit | Failure log `30922915073`; workflow fix pending CI verification |
