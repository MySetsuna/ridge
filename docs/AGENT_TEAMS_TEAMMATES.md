...
**问题**：Ridge 是否真支持 Claude Code 的 Agent Teams 模式自动把
...
tmux pane 跑 `claude --teammate ...`"。Ridge 通过 **tmux shim 二进制
+ 后端 HTTP API + AppState pane tree** 把这个抽象映射到真实 Ridge pane：

```
Claude Code (parent process, 在 Ridge 终端 pane 里运行)
    │
    │ exec("tmux split-window -h ...")
    ▼
src-tauri/src/bin/tmux.rs ────（shim binary，安装到 dist/teammate-shim/tmux）
    │
    │ POST /api/v1/split-window  (HTTP @ RIDGE_TEAMMATE_URL)
    ▼
...
### 已映射的 tmux 命令（shim → API → Ridge）

| tmux command          | shim handler               | backend route               | 真实 Ridge 行为                                      |
|-----------------------|----------------------------|-----------------------------|----------------------------------------------------|
...
| `resize-pane`         | `cmd_resize_pane`          | (shim 内 noop)              | ⚠ 不真 resize（Ridge 用户控制 splitpanes 拖拽）      |
...
cmd: body.command, cwd, ...)`——这是 Ridge 启动一个全新 PTY 进程的标准
...
1. **`new-session` 不真创建会话** — shim 直接返回成功；多 session
   ...
   实际共享同一 Ridge workspace。**用户可见影响**：当前未发现 Claude
...
4. **`new-window` 实际行为** — `route_new_window` 路由存在（line
   154），但实际是把"新 window"翻译成"新 pane"——Ridge 没有 tmux
...
race（第 34 轮已修类似 bug）。
如果 shim 报 "missing RIDGE_TEAMMATE_URL/TOKEN"：shell PATH 没继承
...
的 teammates 自动分屏**。每个 teammate 进入独立的 Ridge pane，pane
头部有 busy 状态标记，PTY 真启动，cwd 继承。其余高级 tmux 行为
（独立 session / kill / resize）按 Ridge 的设计选择不映射，但不阻塞
主流程。
