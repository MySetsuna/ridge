...
- **能力协商**：连接建立时 host/controller 在 hello 交换粗粒度能力集（`pane`/`invoke`/`fs`/`git`/`search`/`workspace`/`theme`/`teammate`），UI 按交集显隐面板；无头 `ridge` host 只暴露缩减集（`pane`/`fs`/`search`），未协商到的能力对应标签自动隐藏
