...
`cargo check` 0 警告；wasm 重建。

**✅ 真 Claude CDP 真机验证通过（2026-06-16）**：dev:cdp 实例跑真 `claude`（不带 NO_FLICKER），CDP 读到
`isInlineTuiMode_LIVE=false`、abs-CSI 已 180s+ 衰减（=旧码会走 shell 路径残留的失败条件），连续 resize
66→28→14(极窄)→52 列**每次都干净**——横幅 Claude 自重折/省略号、输入框完整、**无 ──── 残留、无叠字**，证明
后端粘性 latch 命中、整帧被 wipe。`/exit` 退出 Claude 后 resize 裸 shell（`echo`+`Get-Date`+新提示符）同样
干净，证明粘性已被 prompt OSC/RIS 清除、**shell 无回归**。三修复（A reflow + B 顺序 + C 粘性）合力达成默认模式
Claude resize 不再错位。（box 横幅退出后作为静态 scrollback 残留属固有、任何终端皆然，非回归。）
