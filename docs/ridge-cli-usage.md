...
SUBCOMMANDS:
    login     账号密码登录并直接激活本机（邮箱+密码 → 设用户名 → 绑定设备），免浏览器回环
...
账号密码登录并**直接在终端里激活本机**为远控设备——替代 `rdg remote --enable` 的
「设备码 + 浏览器输配对码」回环。适合已有 Ridge 账号、希望一步把 VPS 接上来的场景。
...
OPTIONS:
    --daemon           激活成功后直接进入守护（等价于随后再跑 `rdg remote --daemon`）
    --shell <SHELL>    指定要拉起的 shell（仅 --daemon 时生效）
    --cwd <CWD>        会话 shell 的工作目录（仅 --daemon 时生效）
    --root <ROOT>      fs 服务根沙箱（仅 --daemon 时生效）[env: RIDGE_REMOTE_ROOT]
    -h, --help         Print help
```

**引导流程**（纯 stdin，无需浏览器）：

1. 读邮箱 + 密码（密码不回显，非 TTY 管道场景回退明文读行），`POST /auth/login` 取 user JWT。
2. 若账号尚无用户名（**试用用户也可设**）→ 提示设置 → `POST /auth/set-username`。
3. 读设备名 → `POST /device/bind`（Bearer user）→ 直接下发 device JWT + 公网入口。
4. 把 device JWT 写入 `~/.config/ridge/auth.json`（与设备码流同一文件 / 形状，Linux 0600）。

登录/设用户名/绑定设备每步最多重试 3 次；用户名占用、设备名重复、设备数超限等给出中文提示。
...
#   密码: ********
#   ✓ 已登录  you@example.com
#     username : alice   plan: TRIAL
#   设备名 (小写字母+数字+连字符, 3–30 位): my-vps
#   ✓ 设备已激活
#     device   : my-vps
#     公网入口 : https://my-vps-alice.9527127.xyz
#     凭据已写入: ~/.config/ridge/auth.json
#   下一步：运行 rdg remote --daemon 开始守护并等待控制端接入。

# 一步登录激活并守护
rdg login --daemon
```

> **接入与激活解耦**：免费 / 试用账号也能登录、设用户名、绑定设备使主机上线（展示为
> 「接入」）；建立**控制通道**仍由云端按订阅门控（见 ridge-cloud `ws/handler.rs`）。
...
```
  ...
  1. 在已登录的浏览器打开:  https://9527127.xyz/activate
  2. 输入下面的配对码 (≈10 分钟内有效):
...
# 步骤 2：首次配对（需要网络 + 浏览器）
rdg remote --enable
# 按提示在浏览器 https://9527127.xyz/activate 输入配对码
...
| `RIDGE_BASE_DOMAIN` | `9527127.xyz` | 覆盖默认服务域名。用于自托管 / 本地测试。影响所有 URL（信令 WS、API、激活页）。 |
...
| `login_flow` | `src/login_flow.rs` | 账号密码登录 + 自助激活（`/auth/login` → `/auth/set-username` → `/device/bind`）+ ANSI banner |
