...
`ridge-cli` 是 Ridge IDE 的**无头远控 host**，设计用于无图形界面的 Linux / VPS 环境。
它的核心价值是：**无需安装 Tauri / WebView / GPU 驱动**，即可把一台服务器变成可被
Ridge 控制端（浏览器或桌面 app）远程控制的终端主机。

### 与统一远控架构的关系

`ridge-cli` 是统一远控架构（`docs/plans/unified-remote-architecture-handoff-final.md`）
的 headless host 实现（子项 **S5**）。架构决策 **D4** 要求 headless host 与桌面 app host
共用同一个**运行时无关** Rust crate（`packages/ridge-core`），而 `ridge-cli` 正是这条
决策的第一个实现载体。
...
**重要**：`ridge-cli` 不依赖 Tauri（`cargo tree -p ridge-cli` 无 `tauri`）。
...
# 产物路径
# Linux / macOS: target/release/ridge-cli
# Windows:       target\release\ridge-cli.exe
```
...
```bash
sudo cp target/release/ridge-cli /usr/local/bin/ridge-cli
```
...
`ridge-cli` 使用 **clap 4** 解析参数。顶层子命令：`remote`（WebRTC 远程控制）与
...
```bash
配对完成。运行 `ridge-cli remote --daemon` 开始守护。
```
...
ridge-cli remote
# stderr:
# 请指定 --enable（配对）或 --daemon（守护）。详见 `ridge-cli remote --help`。
```
...
```bash
ridge-cli tmux --port 47615
# stderr:
# ridge-cli tmux engine listening on http://127.0.0.1:47615
#
...
ridge-cli tmux --port 47615 --token "$(openssl rand -hex 16)"
...
拥有运行 `ridge-cli` 的用户的全部权限。跨机暴露（`--bind 0.0.0.0`）须自行用 firewall /
...
守护运行时，`ridge-cli` 在 **E2EE WebRTC DataChannel** 之上实现了一套二进制帧协议：
...
`ridge-cli` 使用 `CapabilitySet::remote_default()`（`packages/ridge-core/src/capability.rs`），
...
当前 `ridge-cli` 通过 `ridge_core::dispatch` 真实可用的方法：
...
**对 `ridge-cli` 的实际影响**：当前版本通过 `ControlMsg`（`protocol.rs`）只暴露
...
# 以该账户完成配对（一次性）
sudo -u ridge -H /usr/local/bin/ridge-cli remote --enable
