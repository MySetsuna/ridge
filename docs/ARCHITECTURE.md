...
- **状态日期**: 2026-07-25（v0.1.2 发布后：Remote 服务腿收敛 + cloud resync 收敛 + P4 手机保活）
...
- **运行时仅代码/类型级信心**（svelte-check 0-error）；R1–R8 真机验待 R-VERIFY（回退基线 `b9031a0`）。
...
### 2.6 Teammate / Agent 控制面
- `src-tauri/src/teammate/`：`suspend.rs`（软暂停/恢复 + 可选 OS 冻结 Job/SIGSTOP；agent 写路径归一 `agent_pty_write` 唯一收口，suspended 拒；人类输入/Ctrl-C 刻意不门控）、`orch_health.rs`（`get_orchestration_health` = suspended/pending 快照 + `generation`/`degraded`）、`hitl.rs`（`list_hitl_pending` 只读脱敏投影无 action 全文；`resolve_hitl_remote` nonce 单次消费、modify 永不开放）。
...
| R-TESTGATE | P1 | CI 加 test 门（vitest+svelte-check+关键 vite build 冒烟）阻发布 | `release.yml` 含 `vitest run`+`svelte-check`，红则阻断 |
