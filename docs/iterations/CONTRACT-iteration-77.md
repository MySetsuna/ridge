...
- 2026-07-31 production read-only probe: `GET https://9527127.xyz/api/v1/health` returned HTTP `200` with service version `0.0.7`; Remote artifact status remains unverified because `RIDGE_ARTIFACT_TOKEN` is absent. The combined Node probe printed the same health success but then hit a local libuv assertion, so the direct HTTP result is the retained evidence; neither result proves public WebRTC, TURN, artifact freshness, or user-path behavior.
