...
> base 域 = `remo2ridge.duckdns.org`。测试账号 `jxk2yk@gmail.com`（已 premium + is_admin）。
...
curl -sS -X POST https://remo2ridge.duckdns.org/api/v1/auth/request \
  -H 'content-type: application/json' -d '{"client":"desktop"}'

# 管理端 SPA / 授权页（应 200）
curl -sI https://admin.remo2ridge.duckdns.org/ | head -1
curl -sI https://remo2ridge.duckdns.org/authorize | head -1
```

- [ ] 三条都正常。
- [ ] ⚠️ **通配 TLS（最可能的坑）**：浏览器开 `https://test-tester.remo2ridge.duckdns.org/`（语法合法的假租户名，路由不校验设备是否真实存在，会返回 controller SPA）。若**证书不匹配/告警** → 租户子域控制端打不开，需为 `*.remo2ridge.duckdns.org` 签**泛域证书**（Dokku：`dokku letsencrypt` 对 wildcard 需 DNS-01）。
...
- [ ] 专属子域卡片显示 `{device}-{username}.remo2ridge.duckdns.org` + 「**在浏览器打开公网远控**」按钮。
...
## 6. 管理端（admin.remo2ridge.duckdns.org）

- [ ] 开 `https://admin.remo2ridge.duckdns.org/`，用 `jxk2yk@gmail.com` 登录（已 is_admin）。非管理员账号 → 「无管理员权限」，不建立会话。
...
- 排查：
  1. 子域打不开 → 先查**通配 DNS**（`*.remo2ridge.duckdns.org` 是否解析到 Oracle `150.230.171.175`，DuckDNS 默认跟随）+ **通配 TLS 证书**（见 §0）。
