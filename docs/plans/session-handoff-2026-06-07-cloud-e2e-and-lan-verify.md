...
- **B 云完成**：
  ...
  - **B2**（终端经云）✅ **已实现 + 实测验证**。新增 Rust 命令 `subscribe_pane_raw`/`unsubscribe_pane_raw`（新文件 `commands/cloud_pane.rs`，复用 `RemotePtyEvent::RawBytes` raw fan-out → Tauri event `pane-raw-{pane}{b64}`，**未改并发 WIP 的 commands/remote.rs**）+ AppState 订阅表 + **lib.rs fan-out 门控加 `|| cloud_remote_active`**（cloud-only 时 remote_enabled 为 false）。**关键修复**：原 `cloudPaneSource.ts`（订阅 pty-output 事件）对**原生 delta-mode pane 收不到字节**（lib.rs 对 delta 分支 `continue` 跳过了 pty-output 发射）——已删，RemotePanel 改用 `makeCloudHostPaneSource`（走 raw fan-out，在 delta 分支之前，照样推）。实测（dev:cdp 真链路）：cloud 订阅 pane + `write_to_pty('echo …')` → 经 WebRTC 收到完整 PTY 输出（命令回显+结果+OSC7 cwd+prompt，frames=5/bytes=118）。production 由 RemotePanel `notifyCloudActive` 置 cloud_remote_active（已存在）。
  - **B3**（E2EE 公钥↔身份绑定）✅ **已实现 + 实测验证**（commit `7a9199a`）。认证信令旁路确认：两端经信令互报临时公钥→比对 DataChannel 握手公钥，不一致即判 MITM 拒绝。启用门改用「信令公钥到达性」(非 $/hello，后者握手后才发太晚)，宽限期回落 relay-trust 兼容旧端。**relay 透明转发，ridge-cloud 零代码改动**（已实测经运行中 relay 跑通）。实测：正路 `keyBindingMode=enforced` 连通；反路(篡改信令公钥)→拒绝断开。全绿(vitest 682)。详见 `d-gm-10-e2ee-key-binding-design.md` §8。契约文档欠账**已补**：`ridge-cloud/docs/ridge-cloud-protocol.md` §5.3/§7.3 已记 `e2ee-pubkey` 信令消息（ridge-cloud develop `31fdc99`，**本地未推**——推 ridge-cloud develop 会连带域名迁移，留你择机）。
