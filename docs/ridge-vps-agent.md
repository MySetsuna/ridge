...
引擎（`ridge-cli tmux`）背后是与桌面端**逐字节同源**的 `ridge-tmux` native 路由
...
agent 进程 (Claude Code)
  └── 调 `tmux …`（PATH 里是 Ridge 的 tmux shim，src-tauri/src/bin/tmux.rs）
        └── HTTP /api/v1/tmux/*  （读 RIDGE_TEAMMATE_URL / RIDGE_TEAMMATE_TOKEN）
              └── ridge-cli tmux  （监听 RIDGE_TMUX_BIND:RIDGE_TMUX_PORT）
                    └── ridge_tmux::http  ← 与桌面端同一个 native 引擎
```

与 `ridge-cli remote`（WebRTC 远控，连出信令、等浏览器接入）是**两条独立的路**：本文只讲
...
> 没有 Rust 工具链的 VPS：在一台同架构的开发机构建后 `scp` 过去即可——`ridge-cli` 是
...
- `ridge-tmux.service` —— 跑 `ridge-cli tmux` 的服务单元
...
- **无路径沙箱**：引擎对会话内子进程不做路径限制——拉起的 shell 拥有运行 `ridge-cli`
...
[`ridge-cli-usage.md`](./ridge-cli-usage.md) 的 `ridge-cli tmux` 一节。
