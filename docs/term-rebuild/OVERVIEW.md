...
| 3 | WebGPU 后端 + 字形 atlas（替换 Canvas2D）+ 共享 surface | §4.1 ✅ 2026-05-04（17 commit / single session：scaffold + wgpu dep + new()/clear() + GlyphRasterizer + WGSL + 渲染管线 + 资源 + draw_row + atlas eviction + cursor/overlay/underline + AnyBackend dispatch + RenderHandle async constructor）；§4.3 共享 surface + §4.4 perf bench ⏳（依赖 §7.2 实跑验证） |
...
**当前实际位置：round 2.4 → 4/5 → 6/7 → round 3 §4.1 全链路代码层面收尾完毕**（2026-05-04）。**剩余主线工作**：

1. **§7.2 浏览器实跑回归**（需用户）—— 含两条独立验证：(a) §1.18 修复 Canvas2D 实测、(b) WebGPU 路径默认 wasm bundle 实跑。**2026-05-05：WebGPU 进入 cargo `default` features，不再需要 `--features webgpu` 显式指定，也不再读 `localStorage.RIDGE_WEBGPU` 准入门**——`TerminalManager._makeHandle` 在 `attach`/`unpark` 时通过 `typeof RenderHandle.newWithWebgpuFirst === 'function'` 探测、`navigator.gpu.requestAdapter()` 实跑失败时才回退到 Canvas2D。编译期不再做 WebGPU/Canvas2D 二选一，仅运行时降级。
2. **§4.3 共享 surface 跨 pane**（依赖 §7.2 单 pane WebGPU 跑通）。
3. **§4.4 perf benchmark**（依赖 §7.2）。
