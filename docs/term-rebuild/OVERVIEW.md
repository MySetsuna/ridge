...
| — | round 4 部分提前 | ✅ 2026-05-02：鼠标拖选（含 word/line/shift-click）、Ctrl+F 搜索（含 scrollback）、IME v2 cursor-tracking、Ctrl+click 链接；INTEGRATION_R2_4.md 顶部已加 2026-05-03 状态横幅同步该清单 |
| 3 | WebGPU 后端 + 字形 atlas（替换 Canvas2D）+ 共享 surface | §4.1 ✅ 2026-05-04；§4.3 Phase A 共享 GpuContext ✅ 2026-05-06（gpu_context.rs 单例：Instance/Device/Queue/cell_pipeline/GlyphAtlas/sampler 全部跨 pane 复用，atlas_generation 协议处理跨 pane 重建）；§4.3 Phase B 单 Surface ✅ 2026-05-07（surface_host.rs：进程级 wgpu::Surface + scissor 多 pane 复用；+page.svelte 加 `<canvas data-rg-host>`；manager.ts 加 attachHost/resizeHost/_recomputeViewport 与 RAF beginFrame/endFrame；WebGpuPaneBackend.requires_full_frame=true + SurfaceHost.begin_frame 每帧 LoadOp::Clear 解决多缓冲交换链 LoadOp::Load 跨帧不可靠的问题）；§4.4 perf bench ⏳ |
...
**当前实际位置：round 2.4 → 4/5 → 6/7 → round 3 §4.1 + §4.3 Phase A + §4.3 Phase B 全链路代码层面收尾完毕**（2026-05-07）。OVERVIEW.md §2 D1「一个 `<canvas>` 元素覆盖所有 pane 区域，按 scissor rectangle 分区」+「同字号的字形位图全 pane 共用一份」**已交付**。**剩余主线工作**：

1. **§7.2 浏览器实跑回归**（需用户）—— 双 pane / 三 pane 流式 claude + vim + htop 视觉烟囱测试 + CPU 实测：每帧 `surface.getCurrentTexture` / `queue.submit` / `present` 应各 1 次（不论 pane 数），稳态 CPU 与 Phase A 基线相比应再降 1–2%（moth Round 2 §G 估算）。强制 Canvas2D 回退验证：`localStorage.RIDGE_WEBGPU = '0'; location.reload()` 应仍可正常使用。
2. **§4.4 perf benchmark**（依赖 §7.2）。

round 4 的 §2.3 reflow Phase 2 / §2.4 grapheme / §3.3 Bell audio 仍 ⏳ 远期 deferred；TASKS §1.19 元-检查点等所有 ⏳ 关闭后做架构 review。
...
| Resize reflow — Phase 1（live grid，主屏幕） | ✅ 2026-05-03 | `grid.rs::reflow_primary` 已实现，10 条单测全绿；详见 §7「Resize reflow 设计」+ TASKS §2.3 Phase 1 |
...
- **Phase 1 ✅ 2026-05-03**：live grid 列变 reflow，仅主屏幕。
