...
- 2026-05-03 — §1.10 reflow cursor pending_wrap 边界修复：`grid.rs::reflow_primary` 抓 `cursor_src_pending_wrap`，stitch 时把虚拟 past-end 折进 offset（+1）+ post-push clamp 到 trim 后的 pushed_len；引入 `cursor_pending_wrap` 局部 bool，end-of-line `used==0` 分支保留 true，结尾按之赋值（不再无条件 false）。新增 2 单测，113 lib + 22 integ 全绿。— _pending commit_
