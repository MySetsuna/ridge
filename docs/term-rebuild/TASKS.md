...
- 2026-05-03 — OVERVIEW.md §6 风险段同步：R1 测试数 99 → 135、breakdown 92+7 → 113+22，集成场景从 7 条扩到 22 条（新增 ?2026 toggle / ?1004 focus / REP / RIS / OSC 133/633 / OSC 事件顺序等）；R5 状态由「要消除」改「2026-05-03 决议：保留 §5.2 方案 A，深翻走 §2.1 反向 bridge」；「不可逆改动列表」改为「已实施」回顾——Pane.svelte 整文件删除、terminalRegistry.ts 整文件删除、`@xterm/*` 在 `src/` 下 0 引用。
