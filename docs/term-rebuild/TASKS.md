...
- 2026-05-03 — 删除 `commands/git.rs::get_git_info` 死 Tauri command stub：`#[tauri::command]` 标注但 `lib.rs invoke_handler!` 从未注册，前端 `src/` 下 0 处 `invoke('get_git_info', ...)` 引用（Grep 确认），body 仅 `Err("Use get_git_info_with_cwd instead")` 占位。删除 doc comment + `#[allow(dead_code)]` + `#[tauri::command]` + 函数体共 7 行。`get_git_info_with_cwd` 仍为活跃路径并已注册（lib.rs:262）。`cargo check --manifest-path src-tauri/Cargo.toml` 0 错 0 警告。同样套用 CLAUDE.md 新加的规则（justification 引用过期机制时 grep + 删除）。
