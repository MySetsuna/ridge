...
- 2026-05-03 — §1.9 canvas resize 死循环修复：`canvas2d.rs::resize_surface` 不再用 `"{N}px"` 覆写 canvas CSS 宽高（之前会冻结尺寸导致后续 resize 全部 no-op），改写 `"100%"` 让 canvas 持续跟随容器。HTML attrs(device px backing) 仍单独管理。重建 wasm，111 lib + 22 integ 全绿。— _pending commit_
