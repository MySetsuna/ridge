...
- 2026-05-03 — 删除 `state::PaneScrollback::head_seq()` 死方法：注释声称「used by phase-3 scroll-to-tail logic; expose now to keep API stable」，但 Phase 3（§2.1 反向 scrollback bridge）实际走 `get_pane_scrollback_tail` 返回的 `start_seq`/`at_oldest` 字段，never 经 `head_seq`。`Grep head_seq\b|\.head_seq\(` 仅定义点，11 个 state 消费者全部不调用。删除 `#[allow(dead_code)]` + 注释 + 函数体（4 行）。`tail_seq` 仍保留为活跃 API 对偶。`cargo check --manifest-path src-tauri/Cargo.toml` 0 错 0 警告。
