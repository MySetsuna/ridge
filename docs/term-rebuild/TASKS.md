...
- 2026-05-03 — §1.4 sync-output 超时 cool-down：`PaneEntry` 新增 `syncTimeoutRendered`，rAF tick 在超时分支只渲染一次 best-effort frame，之后 `continue` 直到 kernel 退出 sync；kernel 清 sync 时同步 reset。修了原注释承诺却未兑现的「only one render per cycle」语义。svelte-check 0 错误。— _pending commit_
