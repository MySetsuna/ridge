...
### 1.2 [MEDIUM] `feed()` 中 `takePendingEvents()` 条件丢事件 ⏳

- **文件**：`src/lib/terminal/manager.ts:442-445`
- **现象**：`if (entry.eventHandler)` 包住 `takePendingEvents()`。如果初次挂载竞态导致 `feed()` 在 `onEvent()` 之前发生（目前调用顺序保证了不会，但未来重排顺序就会静默丢 OSC 7 cwd 等事件），这批 events 会被 wasm 侧 `mem::take` 直接丢弃，外部不可观测。
- **修法**：无条件 `takePendingEvents()`；handler 缺席时 console.warn（dev only）或暂存。
- **验收**：把 `onEvent` 注册移到 `feed()` 之后，跑一次后续 OSC 7 行为应仍正常。

### 1.3 [MEDIUM] `bellFlashTimer` 未在 `onDestroy` 清理 ⏳
...
- **修法**：`onDestroy` 中 `if (bellFlashTimer !== null) clearTimeout(bellFlashTimer)`。
...
- 2026-05-03 — §1.1 修复 `pane-pty-closed` 重建路径：handler 改 async，依序 `create_pane` + `activate_pane_pty`，每步 guard `alive`；svelte-check 0 错误（2 处 pre-existing 警告）— _pending commit_
