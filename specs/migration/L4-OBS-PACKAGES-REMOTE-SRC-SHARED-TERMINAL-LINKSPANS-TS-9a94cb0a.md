---
id: L4-OBS-PACKAGES-REMOTE-SRC-SHARED-TERMINAL-LINKSPANS-TS-9a94cb0a
level: L4
...
Terminal plain-text link scanning exposes only two actionable classes: structurally valid HTTP(S) web URLs and file-path candidates. Matching uses strict token boundaries, excludes file URLs and non-web schemes, trims sentence delimiters from the visual range, and remains a lazy visible-row scan with no filesystem IO. A path candidate is not actionable until the origin filesystem proves that the CWD/workspace-resolved target is an existing file.
