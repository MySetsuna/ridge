...
describe('ptyBridge.enableDeltaModeThenFit (5b — await-Channel-then-fit)', () => {
	beforeEach(() => {
		vi.useRealTimers();
	});
	afterEach(() => {
	...
	it('AC5.1: fires fit only AFTER setPaneDeltaMode resolves (causal order)', async () => {
		const { ensurePtyBridge, enableDeltaModeThenFit } = await freshBridge();
		await ensurePtyBridge(PANE, WS);

		// Gate set_pane_delta_mode on a deferred promise so the ordering is
		// observable rather than time-dependent.
		let openGate!: () => void;
		...
		const done = enableDeltaModeThenFit(PANE, fit);

		// Flush microtasks: while delta_mode is still pending, fit MUST NOT run.
		await Promise.resolve();
		await Promise.resolve();
		expect(fit).not.toHaveBeenCalled();

		// Open the gate → the await resolves → fit runs strictly after.
		order.push('delta-resolved');
		openGate();
		await done;

		expect(fit).toHaveBeenCalledTimes(1);
		expect(order).toEqual(['delta-resolved', 'fit']);
	});

	it('still fits when the pane has no bridge, and warns (T2-M1 ordering guard)', async () => {
		const { enableDeltaModeThenFit } = await freshBridge();
		const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
		const fit = vi.fn();
		await enableDeltaModeThenFit('unknown-pane', fit);
		// Robustness: the fit is unconditional so a missing bridge can't strand
		// the pane at the default grid (later self-heal / ResizeObserver also run).
		expect(fit).toHaveBeenCalledTimes(1);
		// Observable ordering constraint: calling before ensurePtyBridge surfaces
		// a loud warning instead of silently degrading the deterministic fit.
		expect(warnSpy).toHaveBeenCalledWith(
			expect.stringContaining('enableDeltaModeThenFit called before ensurePtyBridge'),
			expect.objectContaining({ paneId: 'unknown-pane' }),
		);
		warnSpy.mockRestore();
	});

	it('does NOT warn when the bridge is already registered', async () => {
		const { ensurePtyBridge, enableDeltaModeThenFit } = await freshBridge();
		await ensurePtyBridge(PANE, WS);
		const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
		const fit = vi.fn();
		await enableDeltaModeThenFit(PANE, fit);
		expect(fit).toHaveBeenCalledTimes(1);
