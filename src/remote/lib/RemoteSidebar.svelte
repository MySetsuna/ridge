<script lang="ts">
  ...
  let { tab = 'files', cwd = '', workspaceId = '', paneId = '', initialDirectory = '', available, ws, panes = [], attentionPaneIds = [], dataProvider, onClose, onTabChange, onOpenFile, onOpenDiff, onSelectPane, onAttentionChange }: {
    tab?: RemotePanel;
    cwd?: string;
    workspaceId?: string;
    paneId?: string;
...
<div class="sidebar" role="dialog" aria-label="Sidebar">
  ...
  <div class="sb-body">
    {#key `${ws ? remoteSessionId(ws) : 0}:${workspaceId}:${paneId}:${cwd}`}
      {#if tab === 'files'}
        <SidebarFileTree {provider} {onOpenFile} initialPath={initialDirectory} />
      {:else if tab === 'git'}
...
<style>
  ...
  :global(html[data-ridge-pwa="standalone"]) .sb-header{padding-top:64px;min-height:104px}
  @supports (padding-top:env(safe-area-inset-top)) {
    :global(html[data-ridge-pwa="standalone"]) .sb-header{padding-top:max(64px,calc(8px + env(safe-area-inset-top,0px)));min-height:max(104px,calc(48px + env(safe-area-inset-top,0px)))}
  }
