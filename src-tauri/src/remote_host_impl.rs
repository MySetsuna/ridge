...
/// Broadcast the canonical pane grid and its refresh owner to every viewer.
pub(crate) fn broadcast_pane_resize(
    state: &AppState,
    workspace_id: Uuid,
    pane_id: Uuid,
    rows: u16,
    ...
    if let Some(app) = state.app_handle.get() {
        ...
        use tauri::Emitter;
        let _ = app.emit("pty-resized", &payload);
        crate::remote_bridge::forward_event(&app, "pty-resized", payload);
    }
}
        ...
        send
                                ...
                                if subscribed_panes.contains(&(workspace_id, pane_id)) {
                                    let _ = ws_tx
                                        .send(Message::Text(pty_resized_message(
                                            workspace_id, pane_id, rows, cols, owner,
                                        )))
                                        .await;
                                }
                            }
                    ...
                    event = raw_rx.recv() => {
                        match event {
                            ...
                            Some((_, crate::types::RemotePtyEvent::PtyResized { workspace_id, pane_id, rows, cols, owner })) => {
                                if subscribed_panes.contains(&(workspace_id, pane_id)) {
                                    let _ = ws_tx
                                        .send(Message::Text(pty_resized_message(
    ...
    match cmd {
        ...
        "resize_pane" => {
            ...
            let owner = crate::types::PaneResizeOwner::from_wire(
                args.get("owner").and_then(|value| value.as_str()),
                crate::types::PaneResizeOwner::Remote,
            );
...
fn remote_path_rejected(args: &serde_json::Value) -> bool {
    let fields = ["path", "from", "to", "repoRoot", "root", "cwd"];
    fields.iter().any(|key| {
        args.get(key)
            .and_then(|value| value.as_str())
            .is_some_and(path_has_traversal)
    }) || args
        .get("paths")
...
async fn dispatch_allowlisted_invoke(
    cmd: &str,
    args: &serde_json::Value,
    state: &AppState,
    handle: &tauri::AppHandle,
) -> Option<serde_json::Value> {
    // Old cached Remote clients requested desktop-only font data while booting.
    // Return a deterministic immediate error instead of leaving their RPC to
    // expire after 20 seconds; current browser builds never make this request.
    if matches!(
        cmd,
        "load_terminal_font_faces" | "read_terminal_font_face_chunk"
    ) {
        return Some(serde_json::json!({
            "_error": "REMOTE_CLIENT_UPDATE_REQUIRED: refresh Remote to continue"
        }));
    }
