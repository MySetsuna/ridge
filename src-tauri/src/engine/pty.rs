...
pub struct PtyHandle {
    ...
    /// Monotonic nanos deadline; below it we suppress resize-induced
    /// reflow frames so the renderer doesn't thrash.
    pub resize_silence_deadline: Arc<AtomicI64>,
    /// VTE parser; shared across both canonical + legacy paths.
    pub parser: Arc<Mutex<PaneParser>>,
    /// `true` → emit semantic delta frames; `false` → emit raw bytes.
...
#[allow(dead_code)]
pub(crate) struct LocalPtyOwnership {
    pub master: Arc<Mutex<Box<dyn MasterPty + Send>>>,
    pub writer: Arc<Mutex<Box<dyn Write + Send>>>,
    pub _child: Option<Box<dyn portable_pty::Child + Send + Sync>>,
    pub native_ref: Option<(String, usize)>,
