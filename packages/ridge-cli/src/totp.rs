...
//!
//! 与桌面端 `src-tauri/src/remote/auth.rs::RemoteAuth` **算法一致**，确保
//! desktop / cli / 浏览器 controller 三处对同一标准互通：
...
//!
//! 用法（§4）：cli 会话启动时 `RemoteTotp::new()`，把 `current_code()` +
//! `otpauth_uri()` 打到 TUI；E2EE 控制通道上对业务帧门控，收到
//! `{"t":"totp-verify","code":"…"}` 时 `verify()`，回 `{"t":"totp-result","ok":…}`。
//!
//! 零额外依赖：`sha2` 已在本 crate 依赖树（e2ee 用），HMAC 手写（RFC 2104）。

use std::time::{SystemTime, UNIX_EPOCH};

use sha2::{Digest, Sha256};

/// 时间步长（秒）。与桌面端 `TOTP_PERIOD` 一致。
const TOTP_PERIOD: u64 = 30;
/// 验证码位数。与桌面端 `TOTP_DIGITS` 一致。
const TOTP_DIGITS: u32 = 6;
/// 验证窗口：当前步 ±SKEW 都接受（容忍时钟漂移）。与桌面端 `TOTP_SKEW` 一致。
const TOTP_SKEW: i64 = 1;
/// secret 字节数（RFC 6238 推荐 ≥ 输出 HMAC 长度即可；与桌面端一致取 20）。
...
impl RemoteTotp {
    ...
    ///
    /// 注：桌面端 `RemoteAuth::verify` 的窗口是 ±1 **秒**（仅在步边界 1s 内才容
    /// 错），对时钟漂移几乎无容忍——那是偏紧的。这里取标准 ±1 **步**（前后各一个
    /// 30s 窗口），更贴合「上一个码还能用一会儿」的用户预期。**码生成算法/周期/
    /// 位数与桌面端逐字节一致**，跨实现产出的 code 相同；窗口只是本机放行的宽容度，
    /// 不影响线缆兼容性。
...
/// 随机 20 字节 secret。用 `rand`（已是本 crate 依赖）的 OS 熵源。
...
/// HMAC-SHA256（RFC 2104）。手写以避免引额外 crate（与桌面端逐字节一致）。
...
mod tests {
    ...
    /// 本实现取 6 位，即 8 位标准值的末 6 位 `119246`。这锁死算法/截断/位数
    /// 与标准一致，桌面端与浏览器 controller 才能对得上。
    #[test]
    fn rfc6238_known_vector_sha256() {
        let secret = b"12345678901234567890123456789012"; // 32 字节 ASCII
                                                          // 8 位标准值 46119246 截到 6 位 = 119246（mod 1_000_000）。
        assert_eq!(totp_at(secret, 59), "119246");
    }

    /// current_code() → verify() 往返：自洽。
    #[test]
    ...
    fn far_step_rejected() {
        ...
        // 三步之外不在 ±1 窗口；除非极小概率两步 code 撞码，否则应被拒。
        assert!(!totp.verify(&far) || far == totp.current_code());
    }

    /// otpauth URI 形态：含 SHA256、6 位、30s 周期与 base32 secret。
    #[test]
