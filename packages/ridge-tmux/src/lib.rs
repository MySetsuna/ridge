...
// ── Optional teammate-protocol HTTP surface (feature = "http") ──
// One axum router over the native engine, mounted by BOTH the desktop teammate
// server and the headless `ridge-cli` host so the `/api/v1/tmux/*` handlers
