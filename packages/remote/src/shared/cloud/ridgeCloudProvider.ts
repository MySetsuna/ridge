...
export interface RidgeCloudHostCallbacks {
  ...
  /** Owner-host second gate for relay workspace-scope assertions. */
...
export class RidgeCloudHost {
  ...
  private onDataChannelMessage(conn: ControllerConn, data: unknown, lane: 'control' | 'pane' = 'control'): void {
    ...
    this.handleEncryptedFrame(conn, bytes, lane);
  }

  private handleHandshake(conn: ControllerConn, bytes: Uint8Array): void {
    try {
  ...
  private handleEncryptedFrame(
    ...
    try {
      ...
      if (conn.bridge) conn.bridge.handleFrame(plaintext);
    } catch (error) {
      this.fail(error instanceof Error ? error.message : '收到无法解密的帧（已丢弃）', 'FORBIDDEN');
    }
  }
  ...
  private decideBinding(conn: ControllerConn, graceExpired = false): void {
    ...
    if (decision === 'accept') {
      ...
      void this.acceptConn(conn, conn.pendingHandshakePub);
    } else {
  ...
  private async acceptConn(conn: ControllerConn, peerPub: Uint8Array): Promise<void> {
    ...
    if (!this.conns.has(conn.cid)) return;
  ...
  private async onSignal(raw: unknown): Promise<void> {
    if (typeof raw === 'string' && this.handleWorkspaceScope(raw)) return;
    const parsed = parseSignal(typeof raw === 'string' ? raw : '');
    if (isInboundSignal(parsed)) await this.handleSignalMessage(parsed);
  }

  private handleWorkspaceScope(raw: string): boolean {
    try {
      const scope = JSON.parse(raw) as { t?: unknown; cid?: unknown; token?: unknown };
      if (scope.t !== 'workspace-scope') return false;
      if (
        typeof scope.cid !== 'string' ||
        typeof scope.token !== 'string' ||
        scope.token.length === 0 ||
        scope.token.length > 8192
      ) return true;
      const conn = this.ensureConn(scope.cid);
      conn.workspaceScopePromise = this.cb.verifyWorkspaceScope
        ? this.cb.verifyWorkspaceScope(scope.token).catch(() => null)
  ...
  private async handleSignalMessage(msg: SignalIn): Promise<void> {
    switch (msg.t) {
      case 'welcome':
        this.reconnectAttempts = 0;
  ...
  private handlePeerJoin(msg: Extract<SignalIn, { t: 'peer-join' }>): void {
    if (msg.role !== 'controller' || !msg.cid) return;
    if (this.banned.has(msg.cid)) {
      this.sendSignal({ t: 'kick', cid: msg.cid });
      return;
    }
    this.ensureConn(msg.cid);
  }

  private async handleIce(msg: Extract<SignalIn, { t: 'ice' }>): Promise<void> {
    if (!msg.cid || !msg.candidate) return;
    const conn = this.conns.get(msg.cid);
    if (!conn) return;
    try {
      await conn.pc.addIceCandidate(msg.candidate as RTCIceCandidateInit);
    } catch {
      // Optional ICE candidate may arrive after teardown.
    }
  }

  private handlePeerKey(msg: Extract<SignalIn, { t: 'e2ee-pubkey' }>): void {
    if (!msg.cid) return;
    const conn = this.conns.get(msg.cid);
    const key = conn ? base64ToBytes(msg.pubkey) : null;
    if (!conn || !key) return;
    conn.peerSigKey = key;
    this.decideBinding(conn);
  }
