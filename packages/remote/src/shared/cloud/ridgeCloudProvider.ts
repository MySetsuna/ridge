...
export class RidgeCloudHost {
  ...
  private async onSignal(raw: unknown): Promise<void> {
    // workspace-scope is an additive relay event not generated by ridge-signaling yet.
    // Capture it before the strict generated parser. WS message ordering guarantees
    // relay sends this after peer-join and before the controller's first offer.
    if (typeof raw === 'string') {
      try {
        const scope = JSON.parse(raw) as { t?: unknown; cid?: unknown; token?: unknown };
        if (scope.t === 'workspace-scope') {
          if (
        ...
        // Strict parser below owns ordinary malformed-frame handling.
      }
    }
