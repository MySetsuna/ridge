//! Pane 分屏布局树（React Fiber 风格的 reconcile/commit）。
//!
//! 从 `src-tauri/src/engine/pane_tree.rs` 整体移入（D11 Wave A / P1）。纯数据
//! 结构、零 Tauri、零 AppState；桌面经 `engine::pane_tree` re-export 委托，行为
//! 零变化。错误从 `AppError` 改 [`CoreError`]：`PaneNotFound(uuid)` 一一对应
//! （消息逐字 `"Pane not found: {uuid}"`，前端 `.includes("Pane not found")`
//! 抑制依赖它）；原 `PtyError(s)`（split-path 校验类结构错误）→
//! [`CoreError::InvalidArgs`]（无前端字符串匹配）。
//!
//! **`.ridge` 持久化兼容（硬约束）**：`PaneTree`/`PaneNode`/`Pane`/`SplitDirection`
//! 的 serde 表示**逐字不变**（externally-tagged enum：`{"Leaf":…}` / `{"Split":…}`
//! / `"Horizontal"`）；移动时**禁止**加任何 `#[serde(...)]` 标签属性。下方测试含
//! 整树 golden-string 守护。

use crate::error::CoreError;
use crate::workspace::mode::PaneMode;
...
impl PaneTree {
    ...
    pub fn split(&mut self, target_id: Uuid, direction: SplitDirection) -> Result<Uuid, CoreError> {
        ...
        if let Some(new_root) = Self::reconcile_split(&self.root, target_id, direction, new_pane_id)
        {
            ...
            Err(CoreError::PaneNotFound(target_id))
        }
    }
    ...
    pub fn set_split_ratios_at_path(
        ...
        let PaneNode::Split { ratios: rs, .. } = node else {
            return Err(CoreError::InvalidArgs("target is not a split".into()));
        };
        *rs = next;
        Ok(())
    }
    ...
    fn validate_and_normalize_ratio_update(
        ...
        let sum: f32 = ratios.iter().sum();
        if sum <= 1e-6 {
            return Err(CoreError::InvalidArgs("ratios sum is zero".into()));
        }
        Ok(ratios.iter().map(|r| (*r / sum) * 100.0).collect())
    }

    fn split_at_path<'a>(node: &'a PaneNode, path: &[usize]) -> Result<&'a PaneNode, CoreError> {
        let mut cur = node;
        for &idx in path {
            match cur {
                PaneNode::Split { children, .. } => {
                    cur = children.get(idx).ok_or_else(|| {
                        CoreError::InvalidArgs(format!(
                            "split path out of range at child index {idx}"
                        ))
                    })?;
                }
                _ => {
    ...
    fn mut_split_at_path<'a>(
        node: &'a mut PaneNode,
        path: &[usize],
    ) -> Result<&'a mut PaneNode, CoreError> {
        let mut cur = node;
        for &idx in path {
            match cur {
                PaneNode::Split { children, .. } => {
    ...
    pub fn detach_leaf(&mut self, pane_id: Uuid) -> Result<(), CoreError> {
        let leaves = self.get_all_leaves();
        if leaves.len() <= 1 {
            return Err(CoreError::InvalidArgs("Cannot detach the only pane".into()));
        }
    ...
    pub fn swap_leaves(&mut self, a: Uuid, b: Uuid) -> Result<(), CoreError> {
        if a == b {
            return Ok(());
        }
        let leaves = self.get_all_leaves();
        if !leaves.contains(&a) {
            return Err(CoreError::PaneNotFound(a));
        }
        if !leaves.contains(&b) {
            return Err(CoreError::PaneNotFound(b));
        }
    ...
    pub fn dock_pane(
        ...
        self.panes
            .get(&source)
            .ok_or(CoreError::PaneNotFound(source))?;
        self.panes
            .get(&target)
            .ok_or(CoreError::PaneNotFound(target))?;
        ...
        match region {
            ...
            DockRegion::Left | DockRegion::Right | DockRegion::Top | DockRegion::Bottom => {
                ...
                if !leaves.contains(&source) {
                    return Err(CoreError::PaneNotFound(source));
                }
    ...
    pub fn attach_external_leaf(
        ...
        if !self.panes.contains_key(&target_id) {
            return Err(CoreError::PaneNotFound(target_id));
        }
        let new_subtree = match region {
            DockRegion::Left => PaneNode::Split {
                direction: SplitDirection::Horizontal,
...
mod tests {
    ...
    // ── D11 Wave A serde golden（守 `.ridge` 落盘形态；review-D 指出旧测试仅覆盖 Pane）──

    #[test]
    fn split_direction_serde_is_externally_tagged() {
        assert_eq!(
            serde_json::to_string(&SplitDirection::Horizontal).unwrap(),
            "\"Horizontal\""
        );
        assert_eq!(
            serde_json::to_string(&SplitDirection::Vertical).unwrap(),
            "\"Vertical\""
        );
    }

    #[test]
    fn pane_node_leaf_serde_golden() {
        let node = PaneNode::Leaf(Uuid::parse_str("00000000-0000-0000-0000-000000000001").unwrap());
        assert_eq!(
            serde_json::to_string(&node).unwrap(),
            r#"{"Leaf":"00000000-0000-0000-0000-000000000001"}"#
        );
        let back: PaneNode =
            serde_json::from_str(r#"{"Leaf":"00000000-0000-0000-0000-000000000001"}"#).unwrap();
        assert!(matches!(back, PaneNode::Leaf(_)));
    }

    #[test]
    fn pane_node_split_serde_golden() {
        let node = PaneNode::Split {
            direction: SplitDirection::Horizontal,
            children: vec![
                PaneNode::Leaf(Uuid::parse_str("00000000-0000-0000-0000-00000000000a").unwrap()),
                PaneNode::Leaf(Uuid::parse_str("00000000-0000-0000-0000-00000000000b").unwrap()),
            ],
            ratios: vec![50.0, 50.0],
        };
        // externally-tagged Split，字段序 direction/children/ratios，f32 形如 50.0。
        assert_eq!(
            serde_json::to_string(&node).unwrap(),
            r#"{"Split":{"direction":"Horizontal","children":[{"Leaf":"00000000-0000-0000-0000-00000000000a"},{"Leaf":"00000000-0000-0000-0000-00000000000b"}],"ratios":[50.0,50.0]}}"#
        );
        let _back: PaneNode = serde_json::from_str(&serde_json::to_string(&node).unwrap()).unwrap();
    }

    #[test]
    fn pane_tree_full_serde_round_trips_and_is_stable() {
        // 单 pane 树（panes map 单条 → 键序确定）→ to_string 必须逐字等于这份
        // `.ridge` 落盘形态（root 在前 panes 在后；Pane 经 skip_serializing_if 省略
        // cwd/shell_kind）。整树 golden 是 review-D 指出缺失的守护。
        let json = r#"{"root":{"Leaf":"00000000-0000-0000-0000-000000000001"},"panes":{"00000000-0000-0000-0000-000000000001":{"id":"00000000-0000-0000-0000-000000000001","mode":"Terminal"}}}"#;
        let tree: PaneTree = serde_json::from_str(json).unwrap();
        assert_eq!(tree.get_all_leaves().len(), 1);
        assert_eq!(serde_json::to_string(&tree).unwrap(), json);
    }

    // ── 错误映射：PaneNotFound 消息逐字（前端 .includes 依赖），结构错误为 InvalidArgs ──

    #[test]
    fn split_on_missing_target_is_pane_not_found_verbatim() {
        let mut tree = PaneTree::new();
        let missing = Uuid::parse_str("00000000-0000-0000-0000-0000000000ff").unwrap();
        let err = tree.split(missing, SplitDirection::Horizontal).unwrap_err();
        assert!(matches!(err, CoreError::PaneNotFound(_)));
        assert_eq!(
            err.to_command_string(),
            format!("Pane not found: {missing}")
        );
    }

    #[test]
    fn bad_split_path_is_invalid_args() {
        let mut tree = PaneTree::new(); // root 是 Leaf，不是 Split
        let err = tree
            .set_split_ratios_at_path(&[0], vec![100.0])
            .unwrap_err();
