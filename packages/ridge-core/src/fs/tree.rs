//! File-tree walker — migrated verbatim from `src-tauri/src/fs/tree.rs` (S5).
//!
//! Pure logic: `std::fs` + the `ignore` crate's gitignore matcher. **Zero
//! Tauri dependency** — the desktop host re-exports these types from
//! `src-tauri/src/fs/mod.rs`, and the headless `ridge-cli` host reaches them
//! through `ridge_core::dispatch`. Behaviour is byte-for-byte identical to the
//! pre-migration desktop module (the tests below are the same ones that lived
//! in src-tauri).
